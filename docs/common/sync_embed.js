  ...
  function openPanel(){
    ...
    card.innerHTML=`<h3 style="margin:0 0 8px">☁️ Google Drive — <b>${u}</b> (<span id="kgb_kind">${kind}</span>)</h3>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin:6px 0">
        ...
        <span id="gd_status" style="margin-left:auto;color:#334155">Status: onbekend</span>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin:6px 0">
        <a href="${URLS.full}" target="_blank">Open Full</a>
    ...
    q('#gd_login').onclick=async ()=>{ try{ toast('Inloggen…'); await requestToken(true); setStatus('ingelogd'); toast('Ingelogd'); }catch(e){ setStatus('fout'); toast('Fout: '+e.message); } };
    q('#gd_logout').onclick=()=>{ clearAuth(); setStatus('uitgelogd'); toast('Uitgelogd'); };

    q('#gd_push').onclick=async ()=>{ try{ toast('Bezig: Drive Push…'); await ensureAuthed(); await drivePush(); setStatus('ingelogd'); }catch(e){ setStatus('fout'); toast('Fout: '+e.message); } };
    q('#gd_pull').onclick=async ()=>{ try{ toast('Bezig: Drive Pull…'); await ensureAuthed(); await drivePull(); setStatus('ingelogd'); }catch(e){ setStatus('fout'); toast('Fout: '+e.message); } };
    q('#gd_open').onclick=async ()=>{ try{ await ensureAuthed(); await driveOpen(); setStatus('ingelogd'); }catch(e){ setStatus('fout'); toast('Fout: '+e.message); } };
