...
function buildSwatches(){ const box=$("#swatches"); box.innerHTML=""; pastel.forEach(c=>{ const b=document.createElement("button"); b.style.background=c; b.title=c; b.addEventListener("click",()=>{ const target=$("#themeTarget").value; theme[target=== "bg" ? "bg": target==="card" ? "card": target==="acc" ? "acc": target==="grid" ? "grid":"txt"]=c; applyTheme(theme)}); box.appendChild(b); }); $("#intensity").value=theme.intensity; $("#gridIntensity").value=theme.gridIntensity}
...
$$(".tab").forEach(b=>b.addEventListener("click",()=>{
  $$(".tab").forEach(x=>x.classList.remove("active")); b.classList.addClass?.("active")||b.classList.add("active");
